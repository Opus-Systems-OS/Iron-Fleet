...
| `GET /sessions/{id}/events?page=&limit=&types=&order=` | Proxies `GET /v1/sessions/{id}/events`: the session's event history, oldest first (`order=desc` for newest first), Anthropic envelope (`data`, `next_page`, `prev_page`) unchanged. `types` is comma-separated and fanned out to the API's repeated `types[]`. `order=desc&types=agent.message&limit=1` is the session's latest reply. |
...
| `POST /sessions/{id}/tool-results` | `{results: [{custom_tool_use_id, content, is_error?}]}` → one `user.custom_tool_result` per entry, answering the session's `agent.custom_tool_use` events so a turn waiting with `stop_reason: requires_action` continues. |
